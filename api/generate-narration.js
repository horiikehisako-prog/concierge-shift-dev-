...
const splitNarrationSafely = (text, ratio = 0.35) => {
...
const parseNarrationTextFallback = content => {
  ...
  if (paragraphs.length >= 4 && text.length >= 200) {
    const midpoint = Math.ceil(paragraphs.length * 0.35);
    return {
  ...
  if (lines.length >= 4 && text.length >= 120) {
    const midpoint = Math.ceil(lines.length * 0.35);
    return {
...
const buildSystemPrompt = extraInstruction => [
  ...
  "Opening narration should be 30-40% of the total. It reflects on life, personality, work or life path, hobbies, family memories, and one memorable episode that shows the deceased's character.",
  "Closing narration should be 60-70% of the total. It begins naturally from the afterglow after the farewell, uses memories or episodes not used in openingNarration, names what remains with the family, and closes quietly.",
...
const buildFastSystemPrompt = extraInstruction => [
  ...
  "Never generate broken Japanese such as 'その笑顔が周りを明るく照らしてくださいましたではなかったでしょうか。', casual fragments such as '歌ったり、踊ったりしていつも可愛い。', or unclear first-person lines such as '私も彼女を見習い、明るく前向きに歩んでいきたい。'.",
  "Balance: [OPENING] must be about 30-40% of the total text. [CLOSING] must be about 60-70%. Opening should be clearly longer.",
  ...
  "Opening length: 330-520 Japanese characters. Closing length: 680-900 Japanese characters. Closing must feel clearly longer.",
