...
const requestNarration = async ({ apiKey, model, temperature, maxTokens, prompt, extraInstruction }) => {
  if (shouldUseResponsesApi(model)) {
    ...
    const callResponses = async forcePlainJson => {
      ...
      const finalSystemPrompt = systemPrompt + " Internal process before final output: STEP1 write the narration; STEP2 correct it as a funeral MC instructor with more than 20 years of experience; STEP3 rewrite it into more natural Japanese; STEP4 read it by ear and remove awkward spoken rhythm; STEP5 output only the completed narration. Do not output step labels, correction notes, alternate drafts, improvement notes, analysis, explanations, or markdown.";
      const body = {
        model,
        input: [
