...
const ensureOpeningFullNameIntro = (value, prompt) => {
  ...
  const fullLabel = `${fullName}様`;
  const fullLabelWithKo = `故${fullName}様`;
  ...
  text = text
    ...
    .replace(new RegExp(`本日、?${fullLabelWithKo}とのお別れの時を迎えました。?`, "u"), "")
...
const buildSystemPrompt = extraInstruction => [
  ...
  "Name rule: after the opening seasonal sentence, use one fixed life-introduction sentence with the full name and no 故 prefix: '{fullName}様は、{age}年という尊いご生涯を閉じ、静かに人生の幕を下ろされました。'. Do not also write '本日、故{fullName}様とのお別れの時を迎えました。'. The closing final line must also use the full name. Everywhere else, use only the given name plus 様 when a name is needed. Never write 故人様 or 個人様 in the narration body.",
  ...
  "Use this opening structure exactly: 1) one refined seasonal sentence ending like この季節 or 頃となりました, 2) '{fullName}様は、{age}年という尊いご生涯を閉じ、静かに人生の幕を下ろされました。', 3) personality shown through episodes, 4) family, 5) hobbies and work or life path if provided, 6) one impressive scene that feels specific to the deceased, 7) close exactly with '尽きることのない感謝の思いを胸に、まもなく開式のお時間でございます。'.",
