...
const buildSystemPrompt = extraInstruction => [
  ...
  "Use this closing structure: 1) begin quietly from the afterglow after the farewell, with a memory not used in openingNarration, 2) the family's feelings, 3) what the deceased left behind, 4) the deceased living on in everyone's hearts, 5) close with the fixed flower-farewell guidance, not with a formal closing declaration. Closing must end exactly with: '{age}年のご生涯に心からの敬意を表し、過ごしてまいりました葬送のひととき。 本日はご会葬いただき、誠にありがとうございました。 これよりは、お花を手向けてのお別れのお時間でございます。 式場内は、お別れの準備へと移らせていただきます。 皆様には、お手荷物をお持ちいただき、後方でお待ちくださいますようお願いいたします。 どうぞよろしくお願いいたします。'. Do not repeat opening content.",
