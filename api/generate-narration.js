...
const ensureOpeningFullNameIntro = (value, prompt) => {
  ...
  text = text
    ...
    .replace(/尊いご生涯/g, "尊い生涯")
    .trim();
  text = text.replace(new RegExp(`故?${escapedGiven}様は、?`, "u"), `${fullLabel}は、`);
  if (!text.slice(0, 220).includes(fullName)) {
    const firstSentence = text.match(/^(.+?[。！？])/u)?.[1] || "";
    const payload = extractPromptPayload(prompt);
    const age = String(payload?.hearingSheet?.age || "").trim();
    const bridge = `${fullLabel}は、${age ? `${age}年という` : ""}尊い生涯を閉じ、静かに人生の幕を下ろされました。`;
    text = firstSentence
...
const buildSystemPrompt = extraInstruction => [
  ...
  "Name rule: after the opening seasonal sentence, use one fixed life-introduction sentence with the full name: '故{fullName}様は、{age}年という尊い生涯を閉じ、静かに人生の幕を下ろされました。'. Do not also write '本日、故{fullName}様とのお別れの時を迎えました。'. The closing final line must also use the full name. Everywhere else, use only the given name plus 様 when a name is needed.",
  ...
  "Use this opening structure exactly: 1) one refined seasonal sentence, 2) '故{fullName}様は、{age}年という尊い生涯を閉じ、静かに人生の幕を下ろされました。', 3) personality shown through episodes, 4) family, 5) hobbies and work or life path if provided, 6) one impressive scene that feels specific to the deceased, 7) close exactly with '尽きることのない感謝の思いを胸に、まもなく開式のお時間でございます。'.",
...
const buildFastSystemPrompt = extraInstruction => [
  ...
  "Do not repeat the given name unnecessarily. After the name appears once, naturally replace it with そのお姿, ご本人, 故人様, その笑顔, or omit the subject where the meaning remains clear. Required fixed closing lines may still use the name.",
...
const compactNarrationPrompt = prompt => {
  ...
  return [
    ...
    "MC perspective: this is not a novel, essay, or profile introduction. Keep the air of 'the MC is speaking quietly in this ceremony hall right now.' Make the manuscript easy for the MC to read and comfortable for attendees to hear.",
    "Name usage: do not repeat the deceased's given name more than necessary. After using the name once, use natural Japanese references such as そのお姿, ご本人, 故人様, その笑顔, or omit the subject where clear. Keep required final lines unchanged.",
